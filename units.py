...
from scipy import exp as _exp, log as _log, log10 as _log10
from scipy import sin as _sin, cos as _cos, tan as _tan
from scipy import arcsin as _arcsin, arccos as _arccos, arctan as _arctan
import re as _re
...
class Unit:
    ...
    def __str__(this):
        if(this == t):
            ...
            return "mol" + _unicode("SUPERSCRIPT MINUS") + _unicode("SUPERSCRIPT ONE")
        elif(this == km / hour):
            return "km/h"
        elif(this == mi / hour):
            return "mph"
...
class Quantity:
    ...
    def toUnit(this, unit):
        if(not(this._unit.isMultiple(Unit(unit)))):
            raise ValueError("Cannot convert quantity in unit {} to quantity in unit {}".format(this._unit, Unit(unit)))
        return Quantity(this._number * this._unit._multiple / Unit(unit)._multiple, unit)
...
mu0 = 1 / (epsilon0 * c**2)
NA = avogadroNumber = Quantity(1).toUnit(mol ** -1)
